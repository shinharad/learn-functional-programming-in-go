...
func UpdateConfigVal(d Datastore, key, val string) (oldValue string) {
	...
	for i := 0; i < valElem.NumField(); i++ { // iterates through every struct type field
		tag := valElem.Type().Field(i).Tag // returns the tag string
		field := valElem.Field(i)          // returns the content of the struct type field
		switch tag.Get("toml") {
		case key:
